...
config_file_path = os.path.join(project_root, 'configurations', 'yaml', 'hyperparameter_tuning', 'cifar10', 'cnn.yaml')
...
class CIFAR10DataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        train_dataset = get_dataset(name='CIFAR10',
                                    train=True,
                                    transform_type=self.transform_type,
                                    size=self.size,
                                    normalize=self.normalize,
                                    flatten=self.flatten)
        val_dataset = get_dataset(name='CIFAR10',
                                  train=False,
                                  transform_type=self.transform_type,
                                  size=self.size,
                                  normalize=self.normalize,
